    ...
    function renderCrucigrama() {
      ...
      const view = document.getElementById('arcade-main-view');
      if(view) {
        view.innerHTML = renderDashboardHTML() + `<button class="game-back-menu" onclick="renderArcadeMenu()">← Volver</button><div class="game-active-header"><span>📝 Crucigrama</span><span>5 Palabras</span></div> tapados \${inputs}<button class="game-play-btn" style="margin-top:25px;" onclick="checkCrucigrama()">Comprobar Respuestas</button>` + obtenerManual();
      }
    }
    ...
    function renderFragmentado() {
      let fHtml = fragObj.arr.map((w, i) => { let isUsed = fragObj.sel.includes(i); return `<div class="frag-word \${isUsed?'used':''}" \${isUsed?'':`onclick="clickFrag(${i})"`} onmouseenter="playMarimbaHover()">\${w}</div>`; }).join('');
      ...
      if(fragObj.sel.length === fragObj.arr.length && fragObj.arr.length > 0) {
        if(currentSentence === fragObj.real) { puntajeJuego++; updatePuntajeGlobal(3); historialRespuestas.push({pre: bancoJuego[ronda].titulo, res: "Correcto", cor: fragObj.real, exp: "Frase ensamblada perfectamente."}); ronda++; nextFragmentado(); } 
        else { updatePuntajeGlobal(-1); historialRespuestas.push({pre: bancoJuego[ronda].titulo, res: "Incorrecto", cor: fragObj.real, exp: "El orden preciso importa."}); ronda++; nextFragmentado(); }
      }
    }
    ...
    function renderWordleBoard() {
      ...
      const view = document.getElementById('arcade-main-view');
      if(view) {
        view.innerHTML = renderDashboardHTML() + `<button class="game-back-menu" onclick="renderArcadeMenu()">← Volver</button><div class="game-active-header"><span>🟩 Wordle Clínico</span><span>Intentos: \${wObj.intentos}/6</span></div><p>Palabra de <b>\${wObj.palabra.length} letras</b>.</p><div style="margin:30px 0;">\${getMutedStyle || histHtml}</div><input type="text" id="w-input" maxlength="\${wObj.palabra.length}" style="text-transform:uppercase;padding:15px;font-size:1.6rem;width:280px;text-align:center;font-weight:bold;"><br /><button class="game-play-btn" onclick="checkWordle()" style="margin-top:25px;">Comprobar Palabra</button>` + obtenerManual();
      }
    }
    ...
    function renderSopa() {
      ...
      for(let r=0; r<8; r++) { for(let c=0; c<8; c++) { let isS = slObj.seleccionadas.some(s => s.r === r && s.c === c); hG += `<div onclick="clickSopa(${r}, ${c})" style="width:45px;height:45px;display:flex;align-items:center;justify-content:center;background:${isS?'#9b59b6':'#fff'};color:${isS?'white':'#333'};border:2px solid #cbd5e1;cursor:pointer;font-weight:900;" onmouseenter="playMarimbaHover()">${slObj.grid[r][c]}</div>`; } }
      hG += '</div>';
      const view = document.getElementById('arcade-main-view');
      if(view) {
